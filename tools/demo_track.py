...
def image_demo(predictor, vis_folder, current_time, args):
    ...
    for frame_id, img_path in enumerate(files, 1):
        ...
        if outputs[0] is not None:
            if args.with_fastreid:
                online_targets = tracker.update(outputs[0], [img_info['height'], img_info['width']], exp.test_size, id_feature=id_feature)  # outputs7维度： x1y1x2y2,置信度，类别置信度，类别
            else:
...
def main(exp, args):
    ...
    model = exp.get_model().to(args.device)  # 检测器
    ...
    if not args.trt:
        ...
        ckpt = torch.load(ckpt_file, map_location="cpu")  # ReID
